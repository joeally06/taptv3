/*
  ...
  1. New Tables
    - conferences (conference details and scheduling)
  ...
  2. Security
    ...
    - Public insert access for nominations
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'update_conferences_updated_at'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_board_members_updated_at'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_hall_of_fame_nominations_updated_at'
  ) THEN
